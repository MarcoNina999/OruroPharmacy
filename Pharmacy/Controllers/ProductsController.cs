...
namespace Pharmacy.Controllers
{
    public class ProductsController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Select(int id, string var1)
        {
            var objProduct = (from s in db.Products where s.Id == id select s);
            var objquantity = objProduct.Select(s => s.Quantity);              
            int quantity = Convert.ToInt32(objquantity.First());
            int quantity1 = Convert.ToInt32(var1);
            Add(quantity,quantity1,id);
            return Json(objProduct, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public void Add(int quantity, int quantity1, int Id)
        {
            quantity1 += quantity;
            db.Database.ExecuteSqlCommand(ResourceSQL.AddProdQuantityName + "@id, @quantity", new SqlParameter("@id", Id), new SqlParameter("@quantity", quantity1));
        }
